...
import { CSPReport, CSPReportsQuery } from "./schemas";
...
export async function getCSPReports(query: CSPReportsQuery) {
  ...
  try {
    ...
    throw new Error(`Erreur lors de la récupération des rapports CSP: ${error}`);
  }
}
...
export async function getCSPStats() {
  try {
    ...
    const stats = {
      ...
      thisWeek: reports.filter(r => new Date(r.createdAt!) >= thisWeek).length,
      byDirective: reports.reduce((acc, report) => {
        const directive = report.reportData?.['csp-report']?.['violated-directive'] || 'unknown';
